...
import { ChevronDown, ChevronUp, DessertIcon, Edit, Loader, Plus, Search, Trash2, Trash2Icon, Users, X } from 'lucide-react'
...
const handleStatusChange = async (jobId) => {
  try {
    const response = await axiosInstance.patch(`/api/jobs/${jobId}/toggle-close`);
    if (response.status === 200) {
      toast.success("Job status updated successfully");
      
      // Update the job status locally instead of refetching all jobs
      setJobs(prevJobs => 
        prevJobs.map(job => 
          job.id === jobId 
            ? { 
...
const getPostedJobs = async (disableLoader) => {
  ...
  try{
    const response = await axiosInstance.get('/api/jobs/get-jobs-employer');

    if(response.status === 200 && response.data?.length > 0){
        const formattedJobs = response.data.map((job) => {
            console.log(`Job "${job.title}" - Original status:`, job.status);
            
            // More robust status mapping
            let mappedStatus;
            if (job.status === "active" || job.status === "Active") {
                mappedStatus = "Active";
            } else if (job.status === "closed" || job.status === "Closed" || job.status === "inactive") {
                mappedStatus = "Closed";
            } else {
                // Fallback for unexpected status values
                mappedStatus = "Closed";
                console.warn(`Unexpected job status: ${job.status} for job: ${job.title}`);
            }
            
            console.log(`Job "${job.title}" - Mapped status:`, mappedStatus);
        ...
        console.log('All formatted jobs:', formattedJobs);
    ...
    if (error.response){
      ...
      console.error("Error fetching jobs. Please try again");
    }
  } finally {
    setIsLoading(false);
  }
...
useEffect(() => {
  ...
  return (
    <DashbordLayout activeMenu='manage-jobs'>
        ...
        <div className='max-w-7xl mx-auto'>
          ...
          <div className='mb-8'>
            <div className='flex flex-row items-center justify-between'>
              ...
              <button
                className='inline-flex items-center px-6 py-3 bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-sm text-white font-semibold rounded-xl shadow-lg shadow-teal-500/25 hover:shadow-xl hover:shadow-teal-500/30 transition-all duration-300 transform hover:-translate-y-0.5 whitespace-nowrap '
            ...
            <div className='flex flex-col sm:flex-row gap-4'>
              ...
              <div className='sm:w-48'>
                <select
                  ...
                  className='block w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all duration-200 bg-gray-50/50 placeholder-gray-400'
                >
                ...
                <div className='w-[75vw] md:w-full overflow-x-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100'>
                  ...
                  <thead className='bg-gradient-to-r from-gray-50 to-gray-100/50'>
                    <tr>
                      ...
                      <th className='px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider min-w-[180px] sm:min-w-0 rounded-tr-2xl rounded-br-2xl'>
                        Actions
                      </th>
                    </tr>
                  </thead>
                  <tbody className='bg-white divide-y divide-gray-200'>
                    {isLoading 
                      ...
                      paginatedJobs.map((job) => {
                        // Debug: Log each job's status when rendering
                        console.log(`Rendering job "${job.title}" with status: ${job.status}`);
                        
                        return (
                        <tr
                          key={job.id}
                          className='hover:bg-teal-50/50 transition-all duration-200 border-b border-gray-100/60'
                        >
                          ...
                          <td className='px-6 py-4 whitespace-nowrap text-sm font-medium min-w-[180px] sm:min-w-0'>
                            <div className='flex space-x-2'>
                              <button
                                className='text-teal-600 hover:text-teal-800 p-2 rounded-lg hover:bg-teal-50 transition-colors duration-200'
                                onClick={()=>navigate("/post-job",{
                                  state: { jobId: job.id },
                                })}
                              >
